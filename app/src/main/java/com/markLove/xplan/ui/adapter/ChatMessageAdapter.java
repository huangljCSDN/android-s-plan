...
public class ChatMessageAdapter extends RecyclerView.Adapter<ChatBaseViewHolder> {
    ...
    public ChatMessageAdapter(Context context, List<Message> datas) {
    ...
    public ChatBaseViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        ...
        switch (ViewType.values()[viewType]) {
            ...
            case SEND_GIFT:
                ...
                break;
            case RECEIVED_GIFT:
                view = LayoutInflater.from(context).inflate(R.layout.chat_received_gift_item, viewGroup, false);
                holder = new ChatGiftViewHolder(view);
                break;
            default:
...
//                    CommonActivity.startPersonalCenter(context, msg.getFromID() + "");
    ...
    static class ChatLoveViewHolder extends ChatBaseViewHolder {
        ...
        public ChatLoveViewHolder(View itemView, ChatMessageAdapter adapter) {
            super(itemView);
            llBecomeLoveContent = (LinearLayout) itemView.findViewById(R.id.chat_become_Content_ll);
            tvBecomeLoveMsg = (TextView) itemView.findViewById(R.id.tv_become_loves_msg);
            llBecomeLove = (LinearLayout) itemView.findViewById(R.id.become_love_ll);
        ...
        public void setChatContent(Message msg) {
            ...
            if (body instanceof FileMessageBody) {
                ...
                if (new File(voicePath1).exists()) {
                    ...
                    if (msg.getFromID() == PreferencesUtils.getInt(context, Constants.ME_USER_ID)) {
                        voicePlayer.setImageResource(R.drawable.send_voice3);
                    } else {
                        voicePlayer.setImageResource(R.drawable.received_voice3);
                    }
                }
            }
        }
                ...
                case VOICE:
                    ...
                    if (msg.getFromID() == PreferencesUtils.getInt(context, Constants.ME_USER_ID)) {
                        ...
                        if (!(boolean) voicePlayer.getTag()) {
                            ...
                            drawable.stop();
                            drawable.selectDrawable(1);
                            voicePlayer.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.received_voice3));
                            voicePlayer.setTag(false);
                        }
                    }
                    break;
